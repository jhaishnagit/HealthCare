...
public class DoctorService {
    ...
    public String uploadDoctorImage(MultipartFile file) throws Exception
 {
        Map uploadResult = cloudinary.uploader().upload(
            file.getBytes(),
            ObjectUtils.asMap("folder", "doctors")
        );
        return uploadResult.get("secure_url").toString();
    }
    public List<Doctor> getTalkAvailableDoctors() {
        return doctorRepository.findByTalkAvailable("YES");
    }

   
    public Page<Doctor> getTalkAvailableDoctors(
            String search,
            int page,
            int size
    ) {
        Pageable pageable = PageRequest.of(page, size);
        return doctorRepository.searchTalkAvailableDoctors(search, pageable);
    }
