...
public class StaffService {
    ...
    public Staff updateStaff(Long id, Staff updatedStaff) {
        return staffRepository.findById(id)
                .map(existing -> {
                    ...
                    existing.setPan(updatedStaff.getPan());
                    existing.setDesignation(updatedStaff.getDesignation());
                    existing.setOrganizationId(updatedStaff.getOrganizationId());
 ...
 // 🔹 Get staff details by email
    ...
    public Staff updateStaffFieldsByEmail(String email, Map<String, Object> updates) {
        ...
        updates.forEach((key, value) -> {
            switch (key) {
                ...
                case "pan" -> staff.setPan((String) value);
                case "designation" -> staff.setDesignation((String) value);
                case "organizationId" -> staff.setOrganizationId(Long.valueOf(value.toString()));
    ...
    public Staff updateStaffFieldsById(Long id, Map<String, Object> updates) {
        ...
        if (updates.containsKey("pan")) {
            staff.setPan((String) updates.get("pan"));
        }
        if (updates.containsKey("designation")) {
            staff.setDesignation((String) updates.get("designation"));
        }
        if (updates.containsKey("organizationId")) {
            staff.setOrganizationId(Long.valueOf(updates.get("organizationId").toString()));
        }
 ...
 // ✅ Check both staff and doctor tables when sending OTP (Forgot Password)
    ...
    public String sendApprovalMail(String email, String orgName, String password) {

        Staff staff = staffRepository.findByEmail(email);
        if (staff == null) {
            throw new RuntimeException("No staff found with email: " + email);
        }

        String ownerName = staff.getName();

        String messageText =
                "Subject: Congratulations — Your Organization Has Been Approved!\n\n" +
                "Hello " + ownerName + ",\n\n" +
                "We are pleased to inform you that your organization, **" + orgName + "**, has been successfully approved by JC Healthcare.\n\n" +
                "Your login credentials:\n" +
                "Email: " + email + "\n" +
                "Password: " + password + "\n\n" +
                "You can now access your dashboard and begin managing your organizational details.\n\n" +
        ...
        try {
            SimpleMailMessage message = new SimpleMailMessage();
            message.setTo(email);
            message.setSubject("Congratulations — Your Organization Has Been Approved!");
            message.setText(messageText);

            mailSender.send(message);
            return "Approval email sent successfully to " + email;
        } catch (Exception e) {
            throw new RuntimeException("Failed to send approval email: " + e.getMessage());
        }
    }

    public String sendRejectionMail(String email, String orgName, String reason) {
        ...
        String messageText =
                "Subject: Application Status Update – JC Healthcare\n\n" +
                "Hello " + name + ",\n\n" +
                "Thank you for submitting your organization, **" + orgName + "**, for verification.\n\n" +
                "After reviewing your application, we regret to inform you that it has not been approved at this time.\n\n" +
                "Reason for Rejection:\n" +
                reason + "\n\n" +
                "We kindly request you to review the submitted details or documents, make the necessary corrections, " +
                "and resubmit your application for further evaluation.\n\n" +
                "If you need any assistance or clarification, feel free to reach out to our support team.\n\n" +
                "Warm regards,\n" +
                "JC Healthcare Team";
        ...
        try {
            SimpleMailMessage message = new SimpleMailMessage();
            message.setTo(email);
            message.setSubject("Application Status Update – JC Healthcare");
            message.setText(messageText);

            mailSender.send(message);
            return "Rejection email sent successfully to " + email;
