...
public class StaffController {
    ...
    public ResponseEntity<?> addStaff(@RequestBody Staff staff) {
        try {
            Staff saved = staffService.addNewStaff(staff);
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Error while saving staff: " + e.getMessage());
        }
    }
    ...
    @PostMapping("/send-approval-mail")
    public ResponseEntity<String> sendApprovalMail(@RequestBody Map<String, String> body) {
        String email = body.get("email");
        String orgName = body.get("organizationName");
        String password = body.get("password");

        if (email == null || orgName == null || password == null) {
            return ResponseEntity.badRequest().body("email, organizationName and password are required");
        }

        try {
            String result = staffService.sendApprovalMail(email, orgName, password);
            return ResponseEntity.ok(result);
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e.getMessage());
        }
    ...
    public ResponseEntity<String> sendRejectionMail(@RequestBody Map<String, String> body) {
        String email = body.get("email");
        String orgName = body.get("organizationName");
        String reason = body.get("reason");

        if (email == null || orgName == null || reason == null) {
            return ResponseEntity.badRequest().body("email, organizationName and reason are required");
        }

        try {
            String result = staffService.sendRejectionMail(email, orgName, reason);
            return ResponseEntity.ok(result);
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(e.getMessage());
        }
